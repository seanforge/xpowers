...
Plan, Implement, and Review remain the fine-grained flow. When the user requests automatic whole-series delivery, `xpowers:stack` owns that outcome and invokes `xpowers:implement` and `xpowers:review` for a completed Plan Series while preserving one cohesive, independently reviewable PR per plan.
...
2. When explicitly invoked for automatic whole-series delivery, `xpowers:stack` delivers the series in dependency order. For each plan, it invokes `xpowers:implement`, then `xpowers:review`, and continues their fix-and-review loop until that PR's current content holds a clean formal review conclusion.
