...
2. The reviewer and finding auditor always occupy different sessions. Any reviewer result with no findings never starts or resumes an auditor. The first findings from a new reviewer round trigger a fresh auditor for that round; further findings from the same reviewer session resume the same auditor. The auditor acts as a brutally skeptical staff engineer and challenges each finding's validity, cause, severity, evidence, and proposed direction without running another independent review.
3. The coordinator freezes the artifact and relays complete evidence between the reviewer and auditor for as many rounds as needed until they reconcile each finding's validity, cause, severity, and fix direction with each other. Only then does the coordinator apply the agreed root-cause fixes and send the complete current change back to the same reviewer. Resume the same auditor only when that review produces findings.
...
5. Claude uses Claude-native subagents with reviewer prompts beginning `/code-review high`, retains their session handles, and never proactively kills reviewer or auditor sessions while the loop may resume them. Codex uses `codex review` and `codex exec`, captures both session IDs, and resumes each session for follow-up turns.
...
Claude and Codex mechanics live in separate, concise references. Each reference describes only how to start the reviewer, conditionally start the auditor, capture their handles, preserve them for the working loop, and resume them. The Claude reference explicitly forbids proactively killing reusable subagent sessions. Codex target selection remains discoverable through CLI help; the reference preserves only the non-obvious session-ID choreography.
...
Use short CLI probes to establish that Codex exposes a review session ID, exposes an auditor thread ID in JSON output, and accepts both IDs through its resume command. Do not perform a substantive review as part of this validation.
