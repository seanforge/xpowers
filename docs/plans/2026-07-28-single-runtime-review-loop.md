...
3. The coordinator freezes the artifact and relays each side's complete arguments, objections, and evidence to the other until both roles have answered every unresolved point and agree on each finding's validity, materiality, cause, severity, and fix direction. Only then is that finding round resolved; the coordinator applies any agreed root-cause fixes and sends the complete current change back to the same reviewer. Resume the same auditor only when that review produces findings.
